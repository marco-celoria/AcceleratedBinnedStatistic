...
def bench(comm, x_cpu, values_cpu, n_bins, max_shared_mem = None, n_threads = 256):
    ...
    if rank == 0:
        ...
        t1_gpu = (cp.cuda.get_elapsed_time(start_gpu_1, end_gpu_1))/(n_iterations) * 1e-3
        ...
        t2_gpu = (cp.cuda.get_elapsed_time(start_gpu_2, end_gpu_2))/(n_iterations) * 1e-3
        ...
        t3_gpu = (cp.cuda.get_elapsed_time(start_gpu_3, end_gpu_3))/(n_iterations) * 1e-3
        ...
        t4_gpu = (cp.cuda.get_elapsed_time(start_gpu_4, end_gpu_4))/(n_iterations) * 1e-3
        ...
        t5_gpu = (cp.cuda.get_elapsed_time(start_gpu_5, end_gpu_5))/(n_iterations) * 1e-3
        ...
        t6_gpu = (cp.cuda.get_elapsed_time(start_gpu_6, end_gpu_6))/(n_iterations) * 1e-3
        
        bench_v1 = benchmark(acc.binned_statistic_v1, (x_gpu, values_gpu, n_bins, n_threads), n_repeat=n_iterations)
        bench_v2 = benchmark(acc.binned_statistic_v2, (x_gpu, values_gpu, n_bins, n_threads), n_repeat=n_iterations)
        bench_v3 = benchmark(acc.binned_statistic_v3, (x_gpu, values_gpu, n_bins, max_shared_mem, n_threads), n_repeat=n_iterations)
        bench_v4 = benchmark(acc.binned_statistic_v4, (x_gpu, values_gpu, n_bins, max_shared_mem, n_threads), n_repeat=n_iterations)
        bench_v5 = benchmark(acc.binned_statistic_v5, (x_gpu, values_gpu, n_bins, max_shared_mem, n_threads), n_repeat=n_iterations)
        bench_v6 = benchmark(acc.binned_statistic_v6, (x_gpu, values_gpu, n_bins, max_shared_mem, n_threads), n_repeat=n_iterations)
    ...
    bench_v1_dist = benchmark(acc.binned_statistic_v1_dist, (comm, local_x_gpu, local_values_gpu, n_bins, n_threads), 
                             n_repeat=n_iterations)
    bench_v2_dist = benchmark(acc.binned_statistic_v2_dist, (comm, local_x_gpu, local_values_gpu, n_bins, n_threads), 
                             n_repeat=n_iterations)
    bench_v3_dist = benchmark(acc.binned_statistic_v3_dist, (comm, local_x_gpu, local_values_gpu, n_bins, max_shared_mem, n_threads), 
                             n_repeat=n_iterations)
    bench_v4_dist = benchmark(acc.binned_statistic_v4_dist, (comm, local_x_gpu, local_values_gpu, n_bins, max_shared_mem, n_threads), 
                             n_repeat=n_iterations)
    bench_v5_dist = benchmark(acc.binned_statistic_v5_dist, (comm, local_x_gpu, local_values_gpu, n_bins, max_shared_mem, n_threads), 
                             n_repeat=n_iterations)
    bench_v6_dist = benchmark(acc.binned_statistic_v6_dist, (comm, local_x_gpu, local_values_gpu, n_bins, max_shared_mem, n_threads), 
                             n_repeat=n_iterations)
    ...
    if rank == 0:
        print(f"Size:{n_samples}\tBins:{n_bins}\tTime 1 GPU  - v1:{t1_gpu:.6f} s \t[{t1_gpu_b:.6f} s]", flush=True)
        print(f"Size:{n_samples}\tBins:{n_bins}\tTime 1 GPU  - v2:{t2_gpu:.6f} s \t[{t2_gpu_b:.6f} s]", flush=True)
        print(f"Size:{n_samples}\tBins:{n_bins}\tTime 1 GPU  - v3:{t3_gpu:.6f} s \t[{t3_gpu_b:.6f} s]", flush=True)
        print(f"Size:{n_samples}\tBins:{n_bins}\tTime 1 GPU  - v4:{t4_gpu:.6f} s \t[{t4_gpu_b:.6f} s]", flush=True)
        print(f"Size:{n_samples}\tBins:{n_bins}\tTime 1 GPU  - v5:{t5_gpu:.6f} s \t[{t5_gpu_b:.6f} s]", flush=True)
        print(f"Size:{n_samples}\tBins:{n_bins}\tTime 1 GPU  - v6:{t6_gpu:.6f} s \t[{t6_gpu_b:.6f} s]", flush=True)
        print(f"Size:{n_samples}\tBins:{n_bins}\tTime {world_size} GPUs - v1:{t1_gpus:.6f} s \t[{t1_gpus_b:.6f} s]", flush=True)
        print(f"Size:{n_samples}\tBins:{n_bins}\tTime {world_size} GPUs - v2:{t2_gpus:.6f} s \t[{t2_gpus_b:.6f} s]", flush=True)
        print(f"Size:{n_samples}\tBins:{n_bins}\tTime {world_size} GPUs - v3:{t3_gpus:.6f} s \t[{t3_gpus_b:.6f} s]", flush=True)
        print(f"Size:{n_samples}\tBins:{n_bins}\tTime {world_size} GPUs - v4:{t4_gpus:.6f} s \t[{t4_gpus_b:.6f} s]", flush=True)
        print(f"Size:{n_samples}\tBins:{n_bins}\tTime {world_size} GPUs - v5:{t5_gpus:.6f} s \t[{t5_gpus_b:.6f} s]", flush=True)
        print(f"Size:{n_samples}\tBins:{n_bins}\tTime {world_size} GPUs - v6:{t6_gpus:.6f} s \t[{t6_gpus_b:.6f} s]\n", flush=True)

...
def _check_errors(errors, checks, n_elements, n_bins):
    ...
    if not checks[6] == 1:
        errors.append(
            f"Distributed v1: Error for {n_elements} elements and {n_bins} bins"
        )
    if not checks[7] == 1:
        errors.append(
            f"Distributed v2: Error for {n_elements} elements and {n_bins} bins"
        )
    if not checks[8] == 1:
        errors.append(
            f"Distributed v3: Error for {n_elements} elements and {n_bins} bins"
        )
    if not checks[9] == 1:
        errors.append(
            f"Distributed v4: Error for {n_elements} elements and {n_bins} bins"
        )
    if not checks[10] == 1:
        errors.append(
            f"Distributed v5: Error for {n_elements} elements and {n_bins} bins"
        )
    if not checks[11] == 1:
        errors.append(
            f"Distributed v6: Error for {n_elements} elements and {n_bins} bins"
        )
...
class TestBinnedStatistics:
    ...
    def test_xs(self):
        ...
        n_samples = 1000
        if self.rank == 0:
            x_cpu = np.random.normal(0.0, 5.0, n_samples) + np.linspace(
                0.0, 5.0, n_samples
            )
            values_cpu = 10 * np.random.uniform(
                low=0.0, high=1.0, size=n_samples
            ) + np.sin(np.linspace(0.0, 5.0, n_samples))
        else:
            x_cpu = np.zeros(n_samples)
            values_cpu = np.zeros(n_samples)
        n_bins = [15, 64, 633]
    ...
    def test_s(self):
        ...
        if self.rank == 0:
            x_cpu = (
                ...
                + np.linspace(0.0, 5.0, n_samples)
                + 5 * np.cos(np.linspace(0.0, 5.0, n_samples))
            )
            values_cpu = (
                10 * np.random.uniform(low=0.0, high=1.0, size=n_samples)
                + np.sin(np.linspace(0.0, 5.0, n_samples))
                + np.linspace(0.0, 5.0, n_samples) ** 2
            )
        else:
            x_cpu = np.zeros(n_samples)
            values_cpu = np.zeros(n_samples)
    ...
    def test_m(self):
        ...
        if self.rank == 0:
            x_cpu = (
                np.random.normal(0.0, 5.0, n_samples)
                + np.linspace(0.0, 1.0, n_samples)
                + 5 * np.cos(np.linspace(0.0, 5.0, n_samples))
                + 2.0
            )
            values_cpu = (
                np.random.uniform(low=0.0, high=10.0, size=n_samples)
                + np.sin(np.linspace(0.0, 5.0, n_samples))
                + np.linspace(0.0, 5.0, n_samples) ** 2
                + 3 * np.random.standard_cauchy(n_samples)
            )
        else:
            x_cpu = np.zeros(n_samples)
            values_cpu = np.zeros(n_samples)

...
def main():
    ...
    n_samples = 40000
    arr_cpu = np.linspace(0., 4., n_samples)
    arr_gpu = cp.asarray(arr_cpu, dtype=cp.float64)
    local_arr_gpu = acc.scatter(comm, arr_gpu)
    
#    print(f"On rank {rank}):\nHead of local_arr_gpu:{local_arr_gpu[:5]}\nTail of local_arr_gpu:{local_arr_gpu[-5:]}", flush=True)

    x_cpu = 0.01 * np.sin(arr_cpu) + arr_cpu**2 + 1.
    values_cpu = arr_cpu + 0.01 * np.cos(arr_cpu) + 2.
    
    local_x_gpu = 0.01 * np.sin(local_arr_gpu) + local_arr_gpu**2 + 1.
    local_values_gpu = local_arr_gpu + 0.01 * np.cos(local_arr_gpu) + 2.
    
    n_bins = 15
    n_threads = 256
    statistic_gpu = acc.binned_statistic_dist(comm, local_x_gpu, local_values_gpu, n_bins)
    print(f"On rank {rank}) Results from acceleratedbinnedstatistic\n{statistic_gpu}", flush=True)
